...
String UNIT = "B01";
long time_pubFirebase=60*1000;     //60s
long time_LogFirebase=10*60*1000;  //10 min
long time_showData=30*1000;        //30s
long time_lcdOff=60*1000;          //60s
...
bool staLCD = true,staShowData = false,staX = true;
int train=0,rest=0;
float temp=0,humid=0,hic=0;
String flag,water;
float iTemp=0,iHumid=0;
...
void loop() {
  ...
  while((staShowData==true)&&((millis()-time0)< time_showData)){
    staX = false;
    ...
    if(WiFi.status() != WL_CONNECTED){
      ...
      wifiManager.autoConnect("HT-"+UNIT);
      Serial.println("connected...yeey :)");
      staShowData = false;

      //Serial.println("Update time and GPS");
      //configTime(TZ_SEC, DST_SEC, "pool.ntp.org");
      //updateData(&display);
  ...
  if(WiFi.status() == WL_CONNECTED){
    if(first==1){
      ...
      updateData(&display);
      drawProgress(&display, 70, "Updating GPS...");   
      maxNetfound = WiFi.scanNetworks();
      updateGPS();      
      first=0;
      
      drawProgress(&display, 80, "Read EEROM");
      if(iTemp!=Firebase.getFloat("ID/"+UNIT+"/Current/iTemp")){
        EEPROM_write(0, String(iTemp));
      }
      if(iHumid != Firebase.getFloat("ID/"+UNIT+"/Current/iHumid")){
        EEPROM_write(10, String(iHumid));
      }
      //iTemp = Firebase.getFloat("ID/"+UNIT+"/Current/iTemp"); 
      //iHumid = Firebase.getFloat("ID/"+UNIT+"/Current/iHumid");      
...
void pubFirebase(String unit_id){     //unit_id: A01,A02,B01,.. 
  ...
  Firebase.setFloat("ID/"+unit_id+"/Current/iTemp", 0.1);
  Firebase.setFloat("ID/"+unit_id+"/Current/iHumid", -0.2);
